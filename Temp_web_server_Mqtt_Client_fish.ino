...
#define FISH_PIN 5
#define delaytime_H (1000UL *60 *60)
#define delaytime_L (1000UL *60 *30)
int fish_state = HIGH;
unsigned long rolltime_H = millis() + delaytime_H;
unsigned long rolltime_L = millis() + delaytime_L;
...
/* 
#define FISH_PIN 5
#define delaytime_H (1000UL *5)
#define delaytime_L (1000UL *2)
int fish_state = 1;
unsigned long rolltime_H = millis() + delaytime_H;
unsigned long rolltime_L = millis() + delaytime_L;
*/
  if (fish_state==HIGH){
    if ((long)(millis()- rolltime_H)>=0) {
      rolltime_H += delaytime_H;
      digitalWrite(FISH_PIN,(fish_state)? HIGH : LOW);
      fish_state = !fish_state;
      rolltime_L = rolltime_H;
      }
  }
  else {
    if ((long)(millis()- rolltime_L)>=0) {
      rolltime_L += delaytime_L;
      digitalWrite(FISH_PIN,(fish_state)? HIGH : LOW);
      fish_state = !fish_state;
      rolltime_H = rolltime_L;
      }
  }
